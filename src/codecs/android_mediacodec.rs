...
fn get_codec_initializers(config: &DecoderConfig) -> Vec<CodecInitializer> {
    ...
    match (
        ...
        (true, CompressionFormat::Heic, _) => {
            // QCOM HEIC decoder is non-pipelined and does not support color conversion;
            // 10-bit to 8-bit decode requests fall back to the generic HEVC decoder.
...
impl MediaCodec {
    ...
    fn initialize_impl(
        ...
        unsafe {
            if !is_grid {
            ...
            } else if config.codec_config.compression_format() == CompressionFormat::Heic {
                // grid + heif
                ...
                if heif_mode == HeifMode::Row {
                    ...
                    helper.update_grid_tile_for_row_mode();
                } else {
                    info!("Setting HEIF mode {}", heif_mode as i32);
                    // Possibly fallback to non-ROW mode decoder after ROW mode decoding failure.
                    // Reset grid_image_helper to initial values if cached, as ROW mode modified it.
                    helper.reset_grid_tile_if_cached();
                }
    ...
    fn enqueue_payloads(
        ...
        while payloads_iter.peek().is_some() {
            ...
            if input_index >= 0 {
                ...
                let res = Self::enqueue_payload_static(
                    codec,
                    input_index,
                    payload,
